...
FILE *pFtemp;//---> указ на  ф ASCII в котор  (edit) редакт слово из базы слов
...
char *pstringedit = NULL; //--> указ на temp строку для редактора перевода из ф *pFtemp
...
/* _2_
1)алф база слов из текста- является рабочей на которой проводится обучение (частотный выбор???)
2)Частотная база слов из текста- по ней проводится самое важное обучение
3)Общая алфавитная база слов в которой хранятся все хоть когда-то отредактированные переводом слова
4)смешанная база слов = общая база + база текста, из которой производится заполнение новых 
слов переводом и редактирование имеющегося перевода при неоходимости

 во время обьединения в алф базу слов из txt сравнивается и переносятся перевод слов из общей,
...
поле стрктуры ru[RU] считывается в пам по указ pstringedit
созд текстовый временн файл
если нет флага редактирования:
открывается ТОЛЬКО пустая строка ru[RU] и переносится текст в edit
если стоит флаг редактирования перевода и строка не нулевая?: переносится текст в edit
после???перед закрытием редактора с сохранением???????????? 
????? считывается строка в pstringedit
удаляется файл??
и ожидается другая команда
*/

// 2__c1__ блок редактирования словаря
puts("\n 1) Если хотите редактировать словарь   - тогда нажмите 'y'  \n");
if ('y' == _getch(stdin))//  ==================================================
{
	pstringedit= (byte*)malloc(RU+1);
	if (pstringedit == NULL)
	{
		printf("Не выделенна память под pstringedit \n");
		system("pause");
		exit(1);
	}
	//printf("%s \n",pmixedcommonsort->en );
	//strncpy(pstringedit, pmixedcommonsort->ru,RU);

	int m;
	for (m = 0; m < numbermixed; m++)
	{
		//pmemsortword[m].repeat =  0;
		printf(" _ %3d.  pmixedcommonsor.id=%3d pmixedcommonsort.en=%3d  _( %s )   \n", m, pmixedcommonsort[m].id, pmixedcommonsort[m].en);    // temp
	}
...
}// end 2с1 редактировать словарь - конец блока редактирования словаря
