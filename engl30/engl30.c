...
struct word *pcommonsort = NULL;  //--> указ на д пам с базовыми словами
struct word *pmixedcommonsort = NULL;  //--> указ на д пам временной смешанной с текстом базы
int numbermemsort;			//количество слов в тексте
...
//_ 2 b __ в увел-ю на число слов текста память переносим в врем common базу для дальн редакт-я
...
// Удачно открытый - считываем в д пам файл базы слов
	...
	numbermixed = numbermemsort + numbercommon;

//выделяем эту д память для смешанной базы и текста ????????????????
pmixedcommonsort = (byte*)malloc(numbermixed);
if (pmixedcommonsort == NULL)
{
printf("Не выделенна память для смешанной базы и текста - pmixedcommonsort \n");
system("pause");
exit(1);
}
